  ...
  <v-app>
    ...
    <v-row justify="center" class="col-12">
      <v-dialog v-model="dialog" persistent max-width="600px">
        <template v-slot:activator="{ on, attrs }">
          <v-btn color="primary" dark v-bind="attrs" v-on="on" class="ma-4">
            Create New Projet
          </v-btn>
        </template>
        <v-card>
          ...
          <v-card-actions>
            ...
            <v-btn color="blue darken-1" text @click="AddProjet"> Add </v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
    </v-row>
    ...
    >
      ...
      <template v-slot:item.action="{ item }">
        <v-btn rounded small color="red" dark @click="deleteProjet(item)">
          Delete
        </v-btn>
        <v-btn
          rounded
...
export default {
  ...
  methods: {
    initialize(num) {
      console.log("init hah", num);
      console.log(db.getters.AllProjects[0]);
    ...
    GetIdSelected(userId) {
      ...
      for (i = 0; i < db.getters.AllUsers.length; i++) {
        var tt =
          db.getters.AllUsers[i].nom + " " + db.getters.AllUsers[i].prenom;
        if (tt == this.manager) {
  ...
  async created() {
    await db.dispatch("Retriev  eProjects")
    console.log(db.getters.AllProjects);

    // this.initialize(2);
    this.tasks = db.getters.AllProjects;

    await db.dispatch("RetrieveUsers");
      var i;
      console.log("userssrsrs",db.getters.AllUsers);
      for (i = 0; i < db.getters.AllUsers.length; i++) {
